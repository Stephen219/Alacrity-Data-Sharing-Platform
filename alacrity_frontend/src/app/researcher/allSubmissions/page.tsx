...
//   researcher__id?: number;
//   full_name? : string;
//   username?: string;
//   researcher__profile_picture?: string;
...
const PublicSubmissions = () => {
  ...
  const toggleBookmark = async (submissionId: number) => {
    try {
      ...
      setBookmarkedSubmissions((prev) =>
        data.bookmarked ? [...prev, submissionId] : prev.filter((id) => id !== submissionId),
      )

      // Updates the local state so the count doesnt require refreshing
      setSubmissions((prev) => prev.map((sub) => (sub.id === submissionId ? { ...sub, bookmark_count } : sub)))
      setFilteredSubmissions((prev) => prev.map((sub) => (sub.id === submissionId ? { ...sub, bookmark_count } : sub)))
    } catch (error) {
      console.error("Bookmark error:", error)
    }
  }
                  ...
                  <div className="flex flex-col h-full">
                    <div className="relative">
                      ...
                      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent p-2">
                        <div
                          ...
                          onClick={(e) => {
                            e.stopPropagation()
                            router.push(`/researcher/profile/${submission.researcher__id}`)
                          }}
                        >
                          {submission.researcher__profile_picture ? (
                            <img
                              src={submission.researcher__profile_picture || "/placeholder.svg"}
                              alt={submission.full_name || "User"}
                              className="w-8 h-8 rounded-full object-cover border-2 border-white"
                            />
                          ) : (
                            <div className="w-8 h-8 rounded-full bg-orange-500 text-white flex items-center justify-center border-2 border-white font-medium">
                              {(submission.full_name || "A")[0].toUpperCase()}
                            </div>
                          )}
                          <div className="ml-2 text-white">
                            ...
                            <div className="text-xs opacity-80">@{submission.username || "user"}</div>
                          </div>
                        </div>
                      </div>
                      <span className="absolute top-2 right-2 bg-black bg-opacity-60 text-white text-xs px-2 py-1 rounded-full flex items-center">
                    ...
                    <div className="p-4 flex flex-col flex-grow">
                      ...
                      <div className="flex justify-between items-center mt-auto">
                        {/* Bookmark Icon */}
                        <div className="flex items-center">
                          <button onClick={() => toggleBookmark(submission.id)} aria-label="Bookmark">
                            <Heart
                              size={24}
                              className={
                                bookmarkedSubmissions.includes(submission.id)
                                  ? "fill-alacrityred text-alacrityred"
                                  : "text-gray-400"
                              }
                            />
                          </button>
                    ...
                    <div className="p-6 md:w-2/3 lg:w-3/4">
                      ...
                      >
                        {submission.researcher__profile_picture ? (
                          ...
                          <div className="w-10 h-10 rounded-full bg-orange-500 text-white flex items-center justify-center border border-gray-200 font-medium">
                            {(submission.full_name || "A")[0].toUpperCase()}
                          </div>
                        )}
                        <div className="ml-3">

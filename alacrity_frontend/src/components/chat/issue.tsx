      ...
      try {
        ...
        const mappedMessages = Array.isArray(messagesData)
          ? messagesData.map((msg) => ({
              message_id: msg.message_id || `${Date.now()}-${Math.random()}`, // Fallback if message_id is missing
              sender: msg.sender,
              content: msg.content,
        ...
        console.log("Mapped messages:", mappedMessages); // Debug to check uniqueness
      } catch (error) {
      ...
      const attemptConnection = () => {
        ...
        socketRef.current.onmessage = (event) => {
          try {
            ...
            if (data.message) {
              const newMessage = {
                message_id: data.message.message_id || `${Date.now()}-${Math.random()}`, // Fallback
              ...
              setMessages((prev) => {
                // Ensure no duplicates by filtering
        ...
        <div className="max-w-4xl mx-auto space-y-4">
          {messages.length === 0 ? (
            ...
            messages.map((message, index) => (
              <div
                key={message.message_id || `${index}-${message.timestamp.getTime()}`} // Fallback key
                className={`flex ${
          ...
          {isTyping && (
            <div className="flex justify-start px-6 py-2" key="typing-indicator">
